...
VOID EvtQueryBatteryParams(_In_ WDFTIMER  Timer)
{
    ...
    WDFIOTARGET_Wrap pdoTarget;
    {
        ...
        if (!NT_SUCCESS(status)) {
            DebugPrint(DPFLTR_ERROR_LEVEL, "Batt: WdfIoTargetCreate failed 0x%x\n", status);
            return;
        }
        ...
        if (!NT_SUCCESS(status)) {
            DebugPrint(DPFLTR_ERROR_LEVEL, "Batt: WdfIoTargetOpen failed 0x%x\n", status);
            return;
        }
    }
    ...
    HID_COLLECTION_INFORMATION collectionInfo = {};
    {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            DebugPrint(DPFLTR_ERROR_LEVEL, "Batt: IOCTL_HID_GET_COLLECTION_INFORMATION failed 0x%x\n", status);
            return;
        }
    ...
    if (!preparsedData) {
        DebugPrint(DPFLTR_ERROR_LEVEL, "Batt: PHIDP_PREPARSED_DATA_Wrap failed\n");
...

    {
        ...
        NTSTATUS status = WdfIoTargetSendIoctlSynchronously(defaultTarget, NULL,
            ...
            NULL, NULL);
        if (!NT_SUCCESS(status)) {
            DebugPrint(DPFLTR_ERROR_LEVEL, "Batt: IOCTL_HID_GET_COLLECTION_DESCRIPTOR failed 0x%x\n", status);
            return;
        }
    }
...

    {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            DebugPrint(DPFLTR_ERROR_LEVEL, "Batt: IOCTL_HID_GET_FEATURE failed 0x%x\n", status);
            return;
        }
